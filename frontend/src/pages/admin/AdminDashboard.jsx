...
import {
  ...
  Search, Download, Edit, Trash2, Bell, Clock, Activity, CheckCircle
} from 'lucide-react';
      ...
      {!loading && activeTab === 'overview' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '-10px' }}>
             <button className="dashboard btn btn-outline" onClick={handleOpenNotifications} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
               <Bell size={18} /> Notifications ({unreadCount})
             </button>
          </div>
          
          <div className="stats-grid">
            <StatCard title="Total Donors" value={donors.length} Icon={Users} colorClass="text-info" />
            <StatCard title="Active Camps" value={camps.length} Icon={Building} colorClass="text-primary" />
            <StatCard title="Blood Units" value={dashboardStats.total_units} Icon={Droplet} colorClass="text-critical" />
            <StatCard title="Pending Requests" value={dashboardStats.pending_requests} Icon={AlertCircle} colorClass="text-warning" />
            <StatCard title="Approved Donations" value={dashboardStats.approved_donations} Icon={CheckCircle} colorClass="text-success" />
            <StatCard title="Today Donated" value={dashboardStats.today_donated_count} Icon={Heart} colorClass="text-primary" />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
            <div className="card">
              <div className="card-header"><h4 className="card-title">Workflow Status Counts</h4></div>
              <div className="card-body">
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {Object.entries(dashboardStats.workflow_status_counts || {}).map(([key, value]) => (
                    <li key={key} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: '8px' }}>
            ...
            <div className="card">
              <div className="card-header"><h4 className="card-title">Top Rejection Reasons</h4></div>
              <div className="card-body">
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {(dashboardStats.rejection_reason_summary || []).map((item) => (
                    <li key={item.rejection_reason} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)', paddingBottom: '8px' }}>
      ...
      {!loading && activeTab === 'donors' && (
        <div className="card">
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="dashboard btn btn-outline" onClick={handleExportDonors} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Download size={18} /> Export CSV
            </button>
          </div>
          <DataTable 
              columns={['ID', 'Name', 'Blood Group', 'Contact', 'Last Donation', 'Status', 'Actions']}
              data={filteredDonors}
              ...
              renderRow={(donor) => (
                  <tr key={donor.id}>
                      ...
                      <td style={{ display: 'flex', gap: '8px' }}>
                          <button className="dashboard btn btn-outline" title="Edit" style={{ padding: '6px' }}><Edit size={16} /></button>
                          <button className="dashboard btn btn-outline" title="Delete" onClick={() => handleDeleteDonor(donor.id)} style={{ padding: '6px', color: 'var(--color-critical)' }}><Trash2 size={16} /></button>
                      </td>
                  </tr>
              )}
          />
      ...
      {!loading && activeTab === 'camps' && (
        <div className="card">
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="dashboard btn btn-outline" onClick={handleExportCamps} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Download size={18} /> Export CSV
            </button>
          </div>
          <DataTable 
              columns={['ID', 'Camp Name', 'Organizer', 'Date', 'Location', 'Expected Donors', 'Actions']}
              data={filteredCamps}
              ...
              renderRow={(camp) => (
                  <tr key={camp.id}>
                      ...
                      <td style={{ display: 'flex', gap: '8px' }}>
                          <button className="dashboard btn btn-outline" title="Edit" style={{ padding: '6px' }}><Edit size={16} /></button>
                          <button className="dashboard btn btn-outline" title="Delete" onClick={() => handleDeleteCamp(camp.id)} style={{ padding: '6px', color: 'var(--color-critical)' }}><Trash2 size={16} /></button>
                      </td>
                  </tr>
              )}
          />
